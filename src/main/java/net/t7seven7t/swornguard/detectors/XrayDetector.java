...
public class XrayDetector implements Listener {
	...
	public void checkRatio(final Player player) {
		final PlayerData data = plugin.getPlayerDataCache().getData(player);
		if (data.getStoneMined() > 150
				&& TimeUtil.getTimeDifference(data.getLastXrayWarn(), System.currentTimeMillis()) > 45000L
				&& ((warnOnIronRatio > 0 && data.getIronMined() > 0 && getIronRatio(player) > warnOnIronRatio) || (warnOnDiamondRatio > 0
						&& data.getDiamondMined() > 0 && getDiamondRatio(player) > warnOnDiamondRatio))) {
			CheatEvent event = new CheatEvent(player, CheatType.XRAY, FormatUtil.format(plugin.getMessage("cheat_message"), 
					player, "xraying"));
